...
function handleClientMessage(ws, data, playerId) {
  ...
  switch (type) {
    ...
    case 'playerAction':
      if (messageData && messageData.type === 'INPUT') {
        logger.debug(`Player ${playerId} sent input sequence: ${messageData.data.sequence}`);
        room.handlePlayerInput(playerId, messageData);
      } else if (messageData && messageData.type === 'HEALTH_UPDATE') {
        room.handlePlayerHealthUpdate(playerId, messageData.data);
      } else {
